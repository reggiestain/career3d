...
)
{
	...
	if ( $paymentmethod === false ) return false;

	$meter = loadMeterInformation( $serialnumber );
	if ( $meter === false ) return false;
	...
	$dtid = insertDebtorTrans(
					$tid,
					$paymentmethod[ 'transactiontype_id' ],
					$account[ 'customerbranch_id' ],
					$created,
					$period_id,
					$reference,
					$salesorder_id,
...
function handleIncomingFunds( &$trans, $use_erp, $transaction_id = 0, $issynctoerpverified = 1 )
{
	...
	if ( $use_erp )
	{
		$rc = handleIncomingFundsERP(
				$tid,
				tget( $trans, "work.created" ),
				tget( $trans, "work.serialnumber" ),
				tget( $trans, "work.paymentmethod_id" ),
				tget( $trans, "work.calc.amount" )
		);
...
)
{
	...
	$dtid = insertDebtorTrans(
					$tid,
					$paymentmethod[ 'transactiontype_id' ],
					$paymentmethod[ 'customerbranch_id' ],
					$created,
					$period_id,
					$reference,
					$salesorder_id,
...
function handleChannelFeesForTenant( &$trans, $vat, $exclusive, $landlord, $use_erp )
{
	...
	tset( $trans, "work.calc.remaining", (
					tget( $trans, "work.calc.remaining" )
					- $vat
					- $exclusive
				)
	);
	...
	if ( $use_erp )
	{
		$rc = handleChannelFeesForTenantERP(
				...
				tget( $trans, "work.created" ),
				tget( $trans, "work.serialnumber" ),
				tget( $trans, "work.incoming_paymentmethod_id" ),
				tget( $trans, "work.calc.amount" )
		);
...
function handleChannelFees( &$trans, $use_erp )
{
	...
	$fees = loadManagementFees(
				tget( $trans, 'params.amount' ),
				tget( $trans, 'work.paymentmethod_id' ),
				tget( $trans, 'work.meter.debtortype_id' ),
				tget( $trans, 'work.meter.country_id' ),
				tget( $trans, 'work.chargefee' ),
				tget( $trans, 'work.account_id' ),
				100,		// tenantfeepercentage. Was hard coded to 100. Still is.
	...
	if ( $newVat < 0.01 ) $newVat = 0.0;
	if ( $newExclusive < 0.01 ) $newExclusive = 0.0;
	if ( $newLandlord < 0.01 ) $newLandlord = 0.0;
...
)
{
	...
	debug_print( "Entry: handleArrears ( ".
		"options = ".(debug_flatten_array($options)).", ".
		"arrear_types = $atypes, ".
		"inclusive = ".(($inclusive_filter)?"true":"false").", ".
	...
	while ($rowf = fdbfetcharray($res))
	{
		...
		if ( $payingamount == 0 ) continue;	// Will this even ever happen?
		...
		// Take the amounts from the Wallet if that is what was requested
		if ( $options[ 'takefromwallet' ] )
		{
			debug_print( "Taking arrears payments from wallet of ${payingamount}" );
			sendToWallet( $trans, '', 'APPROVED', $otid, (-$payingamount), "Transaction #$tid" );
		}
		...
		freakout(
			( $remaining < 0 ),
			"Remaining amount is less than zero. Possibly giving away free money. REPORT IMMEDIATELY ( tid = ".
			($tid). " )",
			true	// fatal until proven otherwise
		);
...
function handleManagementFees( &$trans, $use_erp, $adjust_wallet )
{
	debug_print( "Entry: handleManagementFees ( ".
		"use_erp = ".(($use_erp)?"yes":"no").", ".
		"adjust_wallet = ".(($adjust_wallet)?"yes":"no")." )" );

	handleArrears( $trans,
		$options = array(
			"paymentplan" => false,
			"arrearsfee" => false,
			"takefromwallet" => $adjust_wallet
		),
		$arrear_types = array( 10 ),
		$inclusive_filter = true,
		$use_erp
	);
...
function handleOtherArrears( &$trans, $arrearsfees, $use_erp, $adjust_wallet )
{
...
function handleVend( &$trans, $remaining, $use_erp )
{
